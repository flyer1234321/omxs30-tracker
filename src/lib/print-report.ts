...
export function buildPrintReportHtml(stock: StockData, report: AnalystReport | null) {
  ...
  const stats = [
    ...
    ['VPA', number(stock.epsTrailingTwelveMonths, 2)],
    ...
    ['Mot SMA 125', stock.diffPercent125 == null ? '-' : `${number(stock.diffPercent125)} %`],
    ...
    ['Kvalitet', stock.quality ? `${number(stock.quality.score, 0)} / 10` : '-'],
    ['Stop loss', stock.tradePlan ? number(stock.tradePlan.stopLoss, 2) : '-'],
    ['Riktkurs', stock.tradePlan ? number(stock.tradePlan.target, 2) : '-'],
    ['Risk/vinst', stock.tradePlan ? `${number(stock.tradePlan.rMultiple, 1)}R` : '-'],
  ];
