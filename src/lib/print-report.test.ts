...
const stock: StockData = {
  ...
  volatility: 22, beta: 0.9, maxDrawdown: 18, healthCheck: null,
  currency: 'SEK', atr: 2.5, relativeStrength63: 4.2, earningsTimestamp: null,
  priceToBook: 1.8, bookValue: 68, quality: null, tradePlan: {
...
test('builds a printable report and escapes market data text', () => {
  ...
  assert.match(html, /Läge i intervallet/);
  assert.match(html, /VOLYM/);
  assert.match(html, /chart-section/);
});
